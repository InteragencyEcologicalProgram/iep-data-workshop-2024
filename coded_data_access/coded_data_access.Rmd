...
df_edi_emp_ent <- read_data_entity_names(packageId = edi_emp_id_latest)
df_edi_emp_ent
```
...
raw_emp_2022_edi <- read_data_entity(packageId = edi_emp_id_latest, entityId = edi_emp_ent_id)
...
as_tibble(parameterCdFile) %>% 
  filter(parameter_cd %in% unique(df_srf_uv_data_avail$parm_cd)) %>% 
  select(parameter_cd, parameter_nm, parameter_units) %>% 
  right_join(
    select(df_srf_uv_data_avail, parm_cd, loc_web_ds, begin_date, end_date, count_nu),
    by = join_by(parameter_cd == parm_cd)
  )
```
...
# Join and collect the resulting data frame - collect executes 
